...
// --- ParseConfig error tests ---

func TestParseConfig_TooShort(t *testing.T) {
	t.Parallel()

	_, err := alac.ParseConfig([]byte{0, 0, 0, 0})
	if err == nil {
...
func TestParseConfig_EmptyCookie(t *testing.T) {
	t.Parallel()

	_, err := alac.ParseConfig(nil)
	if err == nil {
...
// --- NewDecoder error tests ---

func TestNewDecoder_InvalidBitDepth(t *testing.T) {
	t.Parallel()

	_, err := alac.NewDecoder(alac.Config{
		FrameLength: 4096,
		BitDepth:    13,
		NumChannels: 2,
		SampleRate:  44100,
...
// --- NewStreamDecoder / Decode error tests on corrupt M4A ---

func TestDecode_EmptyReader(t *testing.T) {
	t.Parallel()

	_, _, err := alac.Decode(bytes.NewReader(nil))
	if err == nil {
...
func TestDecode_GarbageData(t *testing.T) {
	...
	_, _, err := alac.Decode(bytes.NewReader(garbage))
	if err == nil {
...
func TestDecode_TruncatedBeforeMoov(t *testing.T) {
	...
	_, _, err := alac.Decode(bytes.NewReader(truncated))
	if err == nil {
...
func TestDecode_CorruptedStsd(t *testing.T) {
	...
	_, _, err := alac.Decode(bytes.NewReader(corrupted))
	if err == nil {
...
func TestDecode_ZeroedStsz(t *testing.T) {
	...
	pcm, _, err := alac.Decode(bytes.NewReader(corrupted))
	if err != nil {
...
func TestNewStreamDecoder_EmptyReader(t *testing.T) {
	t.Parallel()

	_, err := alac.NewStreamDecoder(bytes.NewReader(nil))
	if err == nil {
		t.Fatal("expected error for empty reader")
	}
