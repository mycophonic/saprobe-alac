...
// naturalFile describes a natural (real) ALAC file for conformance testing.
type naturalFile struct {
Path       string
BitDepth   int
SampleRate int
Channels   int
...
// naturalFiles lists real ALAC files for conformance testing.
// These are probed at runtime to determine format; listed here for documentation.
// Set CONFORMANCE_NATURAL_DIR to a directory containing M4A files to test.
//
//nolint:gochecknoglobals
var naturalFiles = []string{
// Files are discovered from CONFORMANCE_NATURAL_DIR environment variable.
// Format is probed at runtime using saprobe decoder.
}
